...
При старте (`--server`) сервис до ~55 секунд дожидается готовности локальной
Ollama (12 попыток с паузой 5с) вместо разовой проверки — устраняет гонку между
стартом `captainblood.service` и `ollama.service` после ребута VPS: systemd
гарантирует только порядок запуска юнитов, а не то, что Ollama уже слушает порт
11434.
...
Локальная модель Ollama, генерирующая ответы, выбрана из нескольких кандидатов
по качеству (adversarial-тест на путаницу сущностей, анти-галлюцинация) и
скорости на VPS — история сравнения и обоснование текущего выбора в
