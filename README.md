...
внука друзей. Плюс — watchdog с LLM-диагностикой, который восстанавливает сервис при
сбое `/health`, а не просто перезапускает его по таймеру.

Перенесено из `aiexperiment` (ветка `capitan/history-of-blood`, задание Week 6 День 30)
в отдельный Gradle-проект в рамках финального задания челленджа AI Challenge 8 (Week 7
День 35 — "реальная задача с AI в реальном окружении").
...
## Watchdog — LLM-диагностика при сбое (День 35)
