...
class DocumentIndex(private val dbPath: String = "index.db") {
     ...
     *
     * Учитывает ОБЕ стратегии треда — `telegram_thread` и `day_announcement` (Week 6 День 28,
     * см. [TelegramChatReader.buildChunks]) — иначе уже протегированные объявления задания
     * дня считались бы «новыми» при каждой повторной индексации.
     *
