...
/**
 * "удачи" — не голое слово, а конкретная фраза из названия книги "Удачи капитана Блада"
 * (`udaci-kapitana-blada.txt`). Живой баг 2026-07-19: голое "удач" ложно срабатывало на
 * "неудачливых" ("расскажи про самых неудачливых пиратов") — вопрос вообще не про книгу,
 * а модель добавляла нерелевантный тизер про Сабатини в начале и конце ответа.
 */
private val VPS_BOOK_KEYWORDS = listOf("блад", "сабатини", "одиссе", "хроник", "удачи капитана")
 ...
 *
 * Причина (найдено на живом тесте 2026-07-11, тогдашняя embedding-модель — `nomic-embed-text`,
 * с тех пор заменена на `bge-m3`, см. [agent.indexing.EmbeddingClient]): для вопроса "как
 * спасти тонущего в море?" модель дала правильному чанку про безопасность (0.7439) оценку
 * НИЖЕ, чем случайно попавшему не по теме чанку про такелаж (0.7654) — все кандидаты сидели
 * в узком диапазоне 0.74-0.77, и правильный чанк проиграл на долю процента. Для безопасности
 * это неприемлемо: детерминированный keyword-gate надёжнее общего скора (актуально независимо
 * от модели).
 */
